...
static void ft_sja1110_dsa_setup(void *blob)
{
	...
	spi2_path = fdt_get_alias(blob, SPI2_ALIAS);
	if (!spi2_path) {
		pr_err("Alias not found: %s\n", SPI2_ALIAS);
		goto out_err;
	}

	spi2_offset = fdt_path_offset(blob, spi2_path);
	if (spi2_offset < 0) {
		pr_err("Failed to get node offset by path %s. Error: %s\n",
		       spi2_path, fdt_strerror(spi2_offset));
		goto out_err;
	}

	offset = fdt_node_offset_by_compatible(blob, spi2_offset,
					       SJA1110_UC_COMPATIBLE);
	if (offset >= 0)
		fdt_del_node(blob, offset);
	else
		compatible_err(spi2_path, SJA1110_UC_COMPATIBLE, offset);
	...
	if (offset >= 0)
		...
		compatible_err(spi2_path, SJA1110_SWITCH_COMPATIBLE,
			       offset);

	offset = fdt_node_offset_by_compatible(blob, spi2_offset,
					       SJA1110_DSA_COMPATIBLE);
	if (offset < 0) {
		compatible_err(spi2_path, SJA1110_DSA_COMPATIBLE, offset);
		goto out_err;
	}
