...
int init_100base_tx_port1(struct sja_parms *sjap) {
	...
	*/
	...
	// Step 5: apparently there is a code field in the shadow register which determines
	// whether to use slow, typical or fast case:
	val = sja1105_read_reg32_v2(sjap, OTP_CTRL_BASE_ADDR, OTP_SHADOW1_17_ADDR);
	code = (val >> 16) & 0x3;
	...
	if (code == 0x2) { // slow case
		...
		if (ret < 0) {
			printf("Error configuring slow sample trimming\n");
			return ret;
		}
		...
		if (ret < 0) {
			printf("Error configuring fast sample trimming\n");
			return ret;
		}
	} else {
